...
let push = Stack.push
let pop = Stack.pop
let hashmake = Hashtbl.create
let hashset = Hashtbl.replace
let hashfind = Hashtbl.find
let hashash = Hashtbl.mem
...
let symbol_table = hashmake 256

let executereg (thestack: stack_t) (oper: char) (reg: int) =
    try match oper with
        | 'l' ->
            if (hashash symbol_table reg)
            then push (hashfind symbol_table reg) thestack
            else printf "dc: register '%c' (0%o) is empty\n%!"
                 (char_of_int reg) reg

        | 's' -> 
            let newnum = pop thestack
            in hashset symbol_table reg newnum
